...
- Python intra-project package import resolution — absolute dotted imports like `from torch.nn import Module` now resolve to `torch/nn/__init__.py` or `torch/nn.py` when the target exists in the project, converting from `External` (0.60) to `ImportBased` (0.95) for intra-project references (no-ticket)
- `__init__.py` added to directory index file resolution — Python package directories now resolve via `__init__.py` alongside `index.ts`/`index.js` for both Phase 1 import resolution and Phase 2 call resolution (no-ticket)
- Runtime Python import resolver (`src/python_resolver/`) — spawns `python3` subprocess to resolve dotted imports via `importlib` + `inspect.getfile()`, following re-export chains to actual defining files. Feature-gated: `python-resolver`. Requires project to be importable (dependencies installed). Graceful degradation when Python unavailable (no-ticket)
