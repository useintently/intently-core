...
fn expand_globs(root: &Path, patterns: &[String]) -> Vec<PathBuf> {
    ...
    for pattern in patterns {
        ...
        match glob::glob(&pattern_str) {
            ...
            Err(e) => {
                warn!(pattern = %pattern, error = %e, "invalid glob pattern in workspace manifest");
            }
        }
    }
    dirs
    ...
    let parsed: PnpmWorkspaceYaml = match serde_yaml::from_str(&contents) {
        ...
        Err(e) => {
            warn!(
                ...
                error = %e,
                "malformed pnpm-workspace.yaml, falling back to single-project mode"
            );
            return None;
        }
    };
    ...
    let parsed: serde_json::Value = match serde_json::from_str(&contents) {
        ...
        Err(e) => {
            warn!(
                path = %manifest_path.display(),
                error = %e,
                "malformed package.json, falling back to single-project mode"
            );
            return None;
        }
    };
    ...
    let parsed: toml::Value = match contents.parse() {
        ...
        Err(e) => {
            warn!(
                path = %manifest_path.display(),
                error = %e,
                "malformed Cargo.toml, falling back to single-project mode"
            );
            return None;
        }
    };
    ...
    let parsed: toml::Value = match contents.parse() {
        ...
        Err(e) => {
            warn!(
                path = %manifest_path.display(),
                error = %e,
                "malformed pyproject.toml, falling back to single-project mode"
            );
            return None;
        }
    };
