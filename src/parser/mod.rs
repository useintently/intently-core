...
///
/// The parser is created fresh each time because tree-sitter parsers are cheap
/// to instantiate and hold mutable internal state, making them unsuitable for
...
pub fn parse_source(
    ...
    parser.set_language(&ts_language).map_err(|e| IntentlyError::ParseFailed {
    ...
    let tree = parser
        ...
        .ok_or_else(|| IntentlyError::ParseFailed {
            path: path.to_path_buf(),
...
mod tests {
    ...
    fn detects_typescript_extensions() {
        assert_eq!(detect_language(Path::new("app.ts")), Some(SupportedLanguage::TypeScript));
        assert_eq!(detect_language(Path::new("mod.mts")), Some(SupportedLanguage::TypeScript));
        assert_eq!(detect_language(Path::new("mod.cts")), Some(SupportedLanguage::TypeScript));
        assert_eq!(detect_language(Path::new("App.tsx")), Some(SupportedLanguage::Tsx));
    }

    #[test]
    fn detects_javascript_extensions() {
        assert_eq!(detect_language(Path::new("app.js")), Some(SupportedLanguage::JavaScript));
        assert_eq!(detect_language(Path::new("mod.mjs")), Some(SupportedLanguage::JavaScript));
        assert_eq!(detect_language(Path::new("mod.cjs")), Some(SupportedLanguage::JavaScript));
        assert_eq!(detect_language(Path::new("App.jsx")), Some(SupportedLanguage::Jsx));
    }
    ...
    fn parses_typescript() {
        ...
        let parsed = parse_source(&PathBuf::from("t.ts"), source, SupportedLanguage::TypeScript, None).unwrap();
    ...
    fn parses_tsx() {
        ...
        let parsed = parse_source(&PathBuf::from("t.tsx"), source, SupportedLanguage::Tsx, None).unwrap();
        assert!(!parsed.tree.root_node().has_error());
    }
    ...
    fn parses_javascript() {
        ...
        let parsed = parse_source(&PathBuf::from("t.js"), source, SupportedLanguage::JavaScript, None).unwrap();
        assert!(!parsed.tree.root_node().has_error());
    }
...
class UserService:
    def get_user(self, user_id: int):
        pass
"#;
        let parsed = parse_source(&PathBuf::from("t.py"), source, SupportedLanguage::Python, None).unwrap();
        assert!(!parsed.tree.root_node().has_error());
    }
...
"#;
        let parsed = parse_source(&PathBuf::from("t.java"), source, SupportedLanguage::Java, None).unwrap();
        assert!(!parsed.tree.root_node().has_error());
    }
...
"#;
        let parsed = parse_source(&PathBuf::from("t.cs"), source, SupportedLanguage::CSharp, None).unwrap();
        assert!(!parsed.tree.root_node().has_error());
    }
...
"#;
        let parsed = parse_source(&PathBuf::from("t.go"), source, SupportedLanguage::Go, None).unwrap();
        assert!(!parsed.tree.root_node().has_error());
    }
...
"#;
        let parsed = parse_source(&PathBuf::from("t.rs"), source, SupportedLanguage::Rust, None).unwrap();
        assert!(!parsed.tree.root_node().has_error());
    }
...
?>"#;
        let parsed = parse_source(&PathBuf::from("t.php"), source, SupportedLanguage::Php, None).unwrap();
        assert!(!parsed.tree.root_node().has_error());
    }
...
"#;
        let parsed = parse_source(&PathBuf::from("t.rb"), source, SupportedLanguage::Ruby, None).unwrap();
        assert!(!parsed.tree.root_node().has_error());
    }
...
"#;
        let parsed = parse_source(&PathBuf::from("t.kt"), source, SupportedLanguage::Kotlin, None).unwrap();
        assert!(!parsed.tree.root_node().has_error());
    }
...
"#;
        let parsed = parse_source(&PathBuf::from("t.swift"), source, SupportedLanguage::Swift, None).unwrap();
        assert!(!parsed.tree.root_node().has_error());
    }
...
"#;
        let parsed = parse_source(&PathBuf::from("t.c"), source, SupportedLanguage::C, None).unwrap();
        assert!(!parsed.tree.root_node().has_error());
    }
...
"#;
        let parsed = parse_source(&PathBuf::from("t.cpp"), source, SupportedLanguage::Cpp, None).unwrap();
        assert!(!parsed.tree.root_node().has_error());
    }
...
"#;
        let parsed = parse_source(&PathBuf::from("t.scala"), source, SupportedLanguage::Scala, None).unwrap();
        assert!(!parsed.tree.root_node().has_error());
    }
    ...
    fn handles_syntax_error_gracefully() {
        let source = "const x = {{{;";
        let parsed = parse_source(&PathBuf::from("broken.ts"), source, SupportedLanguage::TypeScript, None).unwrap();
        assert!(parsed.tree.root_node().has_error());
    }

    #[test]
    fn language_family_grouping() {
        assert_eq!(SupportedLanguage::TypeScript.family(), LanguageFamily::JavaScriptLike);
        assert_eq!(SupportedLanguage::JavaScript.family(), LanguageFamily::JavaScriptLike);
    ...
    fn compute_input_edit_single_line_change() {
        ...
        assert_eq!(edit.start_position, tree_sitter::Point { row: 0, column: 10 });
    }
    ...
    fn compute_input_edit_insertion() {
        ...
        assert_eq!(edit.start_position, tree_sitter::Point { row: 1, column: 0 });
    ...
    fn compute_input_edit_multiline_insertion() {
        ...
        assert!(edit.new_end_position.row > edit.old_end_position.row
...
"#;
        ...
        // Full parse of v1
        let parsed_v1 = parse_source(&path, source_v1, SupportedLanguage::TypeScript, None).unwrap();
        ...
        old_tree.edit(&edit);
        let incremental = parse_source(&path, source_v2, SupportedLanguage::TypeScript, Some(&old_tree)).unwrap();
